 ...
 * @author Achyut
 */
public class ComputerUsageDetailsReport extends javax.swing.JFrame {
    private String id,from,to;
    private boolean generateAll;
    final Vector data = new Vector();
    final Vector columnNames = new Vector();
    /**
     * Creates new form ComputerUsageDetailsReport
     */
    public ComputerUsageDetailsReport(String id, String from, String to, boolean generateAll) {
 ...
 private void setData(String id, String from, String to, boolean generateAll) {
        this.Jid.setText(id);
        if (generateAll == true) {
            this.fromTitle.setText("Showing all computer records");
            this.Jfrom.hide();
            this.Jto.hide();
            this.jLabel7.hide();
        } else {
    ...
    private void getData(String id, String from, String to, boolean generateAll) {
        ...
        if (generateAll == false) {
                query = "select labcur.lablogincurrentid as id,concat(ifnull(g.name,''),ifnull(f.name,''),ifnull(s.name,''))as name,labcur.logintime as logintime, 'N/A' as logouttime,"
                        ...
                        + " from lablogincurrent as labcur left join student as s on labcur.lablogincurrentid = s.studentid"
                        + " left join guest as g on labcur.lablogincurrentid = g.guestid left join faculty as f on labcur.lablogincurrentid = f.facultyid "
                        + "where labcur.computerid = '"+id+"' and labcur.logintime >='"+from+"'"
                        + " union "
                        + "select labmain.labloginmainid as id,concat(ifnull(g.name,''),ifnull(f.name,''),ifnull(s.name,''))as name,labmain.logintime as logintime, logouttime as logouttime,"
                        ...
                        + " from labloginmain as labmain left join student as s on labmain.labloginmainid = s.studentid"
                        + " left join guest as g on labmain.labloginmainid = g.guestid left join faculty as f on labmain.labloginmainid = f.facultyid "
                        + "where labmain.computerid = '"+id+"' and labmain.logintime >= '"+from+"' and labmain.logouttime <= '"+to+"'"
        ...
        try {
            ...
            Log.log.writeErrorLog(e.getMessage(), "ComputerDetailsReport", "getData");
        }
    }
    /**
    ...
    private void initComponents() {
        ...
        setTitle(": : Computer Usage Details Report : :");
        setResizable(false);
        ...
        jLabel1.setText("Computer Usage Details Report");

        Jid.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        Jid.setText("jLabel3");

        jLabel4.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        jLabel4.setText("Computer ID");
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        Report r = new Report();
        r.computerUsageDetail(id,from,to,generateAll);
